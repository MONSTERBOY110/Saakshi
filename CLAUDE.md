...
- **Phase:** 4 in progress and **uncommitted**. Phases 0 to 3 are committed and pushed by the owner (`5ddbd22`, `f268619`, `74cc0c7`, `c912d69`) and live at https://saakshi-1.vercel.app. Done in Phase 4: **judge-solo mode (P0-10, the last P0)**, **the design pass**, **the second protocol pack (P1-1)** and **the eval harness with a metrics page (P1-2)**. Gate on 2026-09-06: lint, typecheck, **398 unit tests**, build, smoke E2E (6), the design detector (clean), `pnpm eval`, the judge-solo live E2E and golden E2E v3 all pass.
- **Judge-solo shipped:** `pnpm demo:render [pack]` renders a pack's `demo_script` with the `george` voice into `public/demo/advisor/<pack>/*.pcm`, each line checked against what the agent actually said; `lib/audio/mixer.ts`; `lib/demo/synthetic-advisor.ts` (real-time pacing, `wait_for`, waits for a quiet moment, skips a missing line); `lib/session/judge-solo.ts`; the router mixes into the Ears and never the Mouth; `bindKnownSpeaker` calibrates from ground truth; the panel says who you are and what to say next in Roman Hindi; a switch on the setup form, on by default.
- **Design shipped:** a committed visual world chosen by the owner from a direction roll, recorded in `DESIGN.md` with product truth in `PRODUCT.md` and the direction contract in the rendered markup. A called card game on cream stock: the Checkpoint Board is a tabla of named, numbered cards marked with a bean; carnival red is reserved for a prohibited claim; every state carries a drawn mark, a word and a field; `components/marks.tsx` replaces every glyph with authored SVG; motion is a bean drop and a card flip, both disabled under `prefers-reduced-motion`. The README leads with real screenshots captured from a production build into `docs/images/`.
- **Second pack shipped:** `packs/loan-kfs-in.json`, a personal loan under the RBI Key Facts Statement, with eight disclosures, six prohibited claims, its own demo script, customer hints and rendered advisor audio. The start screen switches packs and the product name follows. `tests/unit/loan-pack.test.ts` drives the pack's own script through the engine.
- **Eval shipped:** 70 hand-labelled turns in `eval/corpus/`, scored by `pnpm eval` into `eval/report.json`, rendered at `/metrics` beside the live measurements in `eval/measurements.json`. It found nine real pattern defects, all fixed. The page and the README both say the score is a regression suite, not a generalisation estimate, and both list what is **not** measured.
- **Measured on 2026-09-06:** intervention p50 1368 ms, min 1278 ms, max 1538 ms over ten samples against a 2500 ms budget, with all ten carrying a full end-to-end total; golden v3 1633 ms; judge-solo 2226 ms; analyzer violation precision 1.00; question generation 5.4 s; rule engine on the corpus precision 1.00 and recall 1.00.
- **Known gaps:** live teach-back answer recording is still unexercised, real two-person diarization
  is still owed, Firefox and Safari are untested, and cost per session has not been read off the
  dashboard. All four are stated on `/metrics`. The reply-only badge is **fixed**: the analyzer path
  was starting interruptions without the recogniser lag, so half the runs could only show Saakshi's
  own reply time.
...
- **Next three tasks:** (1) run the deployed URL as a judge would, cold, in a fresh profile, and fix every stumble; (2) Phase 5 submission assets: the MP4, the deck, the 16:9 cover, `docs/submission.md`; (3) optional P1s if time allows: Hindi captions of Saakshi's speech (P1-4) and the PII toggle (P1-5).
