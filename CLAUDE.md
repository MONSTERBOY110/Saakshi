...
- **Phase:** 1 (Ears and the Checkpoint Board) built 2026-09-06, gate met locally, awaiting the owner's commit and push (owner decision: single `main` branch, owner gates every commit and does every push). Phase 0 is committed as `5ddbd22` and **live at https://saakshi-1.vercel.app**. After the push, rerun the smoke and golden E2E against production per `docs/deployment.md` section 6.
- **Phase 1 shipped (uncommitted):** `packs/insurance-ulip-in.json` v1.0.0 (8 checkpoints, 6 prohibited claims with Devanagari, Roman Hindi and English patterns, 7 teach-back topics, demo script) validated by `pnpm packs:validate`; `lib/rules` (normalise, engine with two-turn window and negation guards); `lib/session` (transcript reducer, name-first role calibration, phase machine with audio gates, board, keyterms, Zustand store, controller); `lib/aai` typed Ears (reconnect with gap and turn-order offset) and Mouth clients with a fake-socket test harness; the session room UI (setup form, calibration banner with swap and manual assign, role-coloured transcript with language tags, Checkpoint Board with quotes, citations, severity and a latency slot, agent bar, debug drawer). 160 unit tests, golden fixture `tests/fixtures/golden-turns.json`, golden E2E v1 passing on the two-voice WAV.
- **Shipped (main):** Next.js 15 scaffold with TypeScript strict, Tailwind, shadcn/ui, Zod, Vitest (72 tests), Playwright (smoke + live specs), Prettier, ESLint, GitHub Actions; `/api/token/stt` and `/api/token/agent` (rate limited, verified live); 24 kHz PCM16 worklet capture with resampling fallback, PCM utils, playback with flush; STT URL builder and Zod schemas for every message on both sockets; `/session` spike page with both sockets, captions, latency badge, spike controls and a fixture-exporting debug drawer; 24 recorded fixtures (audio stripped) plus the recorder and splitter; two-voice `tests/fixtures/golden-draft.wav` rendered from the demo script with the offline Windows voices; `docs/decisions.md` with every verified payload shape and spike result. Lint, typecheck, unit tests, build, smoke E2E and the live E2E all pass.
- **Spikes:** S1, S2, S5, S8 pass. S4 fails for `conversation.message` (not visible to the next `reply.create`); the `system_prompt` appendix works and FR-3 is amended accordingly. S6 and S7 pass on synthetic voices (20/22 turns attributed; Hindi arrives in both Devanagari and Roman) and still want a real two-person check. S3 result is in `docs/decisions.md`. S9 (Firefox/Safari) and S10 (Upstash) need the owner.
- **Blocked (owner):** commit the working tree (narrowed `.gitignore`, `docs/deployment.md`, `docs/KICKOFF-PROMPT.md`, gotchas file, status docs) so the docs reach GitHub; real two-person mic check for S6/S7; S9 (Firefox/Safari) and S10 (Upstash, needed by Phase 3).
- **Next three tasks:** (1) owner: commit and push Phase 1, verify production with the smoke and golden E2E, run a real two-person session on `/session` (S6/S7) plus S9/S10; (2) Phase 2 tasks 1 to 4: extend `lib/aai/mouth.ts` with tools and `session.resume`, complete the machine transitions in the controller (INTERVENE, NUDGE), the audio router gating; (3) Phase 2 tasks 5 to 8: `/api/analyze` on the LLM Gateway, fusion and rate limit, the `reply.create` intervention with latency badge, NUDGE on "Saakshi, verify". Original Phase 1 list for reference: (2) Phase 1 task 1: `lib/aai/ears.ts` typed client driven by the recorded fixtures; (3) Phase 1 tasks 2 and 3: role calibration with full-sentence prompts and the diarized transcript UI. Details in `docs/KICKOFF-PROMPT.md`, Phase 1.
