...
- **Phase:** 3 (teach-back and the Consent Certificate) complete on the local gate and **uncommitted**. Phases 0, 1 and 2 are committed and pushed by the owner (`5ddbd22`, `f268619`, `74cc0c7`) and live at https://saakshi-1.vercel.app; Phase 3 is working tree only, waiting on the owner's word to commit. Gate run on 2026-09-06: lint, typecheck, **330 unit tests**, build, smoke E2E (5 passed), live dual-session (1363 frames, none unparseable) and **golden E2E v3 all pass**.
- **Phase 3 shipped:** `lib/cert` (canonical JSON and SHA-256 on Web Crypto in both runtimes, the FR-9 hash chain, the certificate schema and builder, an Upstash store with an in-memory fallback); `/api/certificate`, `/api/certificate/[id]` and `/verify/[id]` with the evidence table, QR and a tampered-copy view; `/api/teachback/questions` with per-session question generation on the LLM Gateway and a protocol-pack fallback; `lib/teachback/questions.ts` (priority by what was flagged, validation that rejects a leading question, a fabricated number, markdown or anything over eighteen words); `lib/session/teachback.ts` (prompt swap, `record_answer`, `reexplain`, progressive reveal of `finish_teachback` in hold mode after three answers, adaptive turn detection at 2200/6000, the advisor guard on Ears diarization with a fifteen second cooldown); `lib/session/tool-results.ts` (results held until `reply.done`, the documented rule, with the hold-mode exception); `lib/session/certify.ts`; `lib/session/mouth-resume.ts` (`session.resume` inside the 30 s window, retries, the silence written into the certificate as a gap); `lib/session/devanagari.ts` (name calibration when the STT answers in Devanagari); the teach-back panel and certificate card; a Finish teach-back control for the operator.
- **Measured on 2026-09-06:** intervention latency p50 1533 ms and p95 1807 ms over ten samples (budget 2500 ms); golden v3 intervention 1703 ms; analyzer violation precision 1.00 on the golden dialogues; teach-back question generation 5.4 s for one call per session, five of five questions kept.
- **Known gap, stated plainly:** the golden WAV cannot exercise live answer recording, so the run reports **0 of 4 answers**. The tools and the guards are covered by unit tests; the live path needs the owner speaking answers, or judge-solo mode in Phase 4. See `docs/decisions.md`.
- **Blocked (owner):** commit Phase 3; a real two-person mic session (S6, S7) which would also exercise the teach-back answers; S9 (Firefox and Safari); S10 (Upstash provisioning, without which certificates live in memory and vanish on redeploy); LLM Gateway model access, still one model at two calls a minute.
- **Next three tasks:** (1) owner: commit and push Phase 3, then provision Upstash so certificates survive a deployment; (2) Phase 4 design pass on the room, the verify page and the landing page, using the design skills the owner listed, plus judge-solo mode so a judge alone can run the whole demo; (3) Phase 5: the video, the deck, the cover and `docs/submission.md`.
